...
class BFLMiner(Miner):
	def __init__(self, device_index, port, options):
		...
		self.last_job = None
		self.min_interval = 9999999999999
		...
		if temperature < self.cutoff_temp:
			...
			if self.is_ok(response):
				...
				if self.is_ok(response):
					...
					say_line('Bad response when sending block data: %s', response)
			else:
				say_line('Bad response when submitting job (ZDX): %s', response)
		else:
	...
	def get_temperature(self):
		...
		if response[0] != b'T' or len(response) < 23 or response[-1:] != b'\n':
			say_line('Bad response for temperature: %s', response)
			return 0
	...
	def check_result(self):
		...
		if response[:12] != 'NONCE-FOUND:' or response[-1:] != '\n':
			say_line('Bad response checking result: %s', response)
			return None
	...
	def mining_thread(self):
		...
		while not self.should_stop:
			...
			if not is_good_init(response):
				say_line('Failed to initialize %s (responded: %s), retrying...', (self.id(), response))
				sleep(5)
				continue
